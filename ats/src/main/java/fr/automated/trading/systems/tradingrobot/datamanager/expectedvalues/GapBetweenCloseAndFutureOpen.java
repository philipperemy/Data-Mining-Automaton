...
public class GapBetweenCloseAndFutureOpen implements BuildExpectedValues {
    ...
    public List<Double> build(CSVData csvData) {
        ...
        for(int i=0; i<csvData.countLines()-1; i++) {
            if(csvData.getValue(i, Constants.CLOSE_IDENTIFIER) < csvData.getValue(i+1, Constants.OPEN_IDENTIFIER))
